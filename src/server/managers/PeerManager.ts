...
export class PeerManager {
	...
	// Removed: urlFailureCounts = new Map<string, number>();
	// Removed: lastUrlFailureCountClearTime: number = 0;
	healthSecret: string;
	...
	private async healthCheck(): Promise<boolean> {
		...
		for (let i = 0; i < retries; i++) {
			try {
				...
				console.error(`Healthcheck attempt ${i + 1} failed with error:`, error);
			}
	...
	// Removed: clearFailedUrlCounts method
	...
	private async processUpdateQueueItem() {
		...
		try {
			...
			console.error(`Error processing update queue item ${url}:`, error);
